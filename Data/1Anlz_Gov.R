#####  Dynamic Regressions with Gov as response variable
{#### Get data
  ...
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Gov, Cap, Pop, Terr, Agri, AgriLag, Infra, Money,
...
{########   
  ...
  {####  Exhaustive regressions with these terms
    ...
    write.csv(output, file="./outputs/1Anlz_Gov_output_1.csv",  row.names=FALSE)
    output <- output[output$delAIC < 2,]
    write.csv(output[1:min(100,nrow(output)),], file="./outputs/1Anlz_Gov_output_2.csv",  row.names=FALSE)
    #write.table(output[1:min(100,nrow(output)),], "clipboard", sep="\t", row.names=FALSE)
  }
}

#######  Regression coefficients
RD <- subset(RegrDat, select = -c(Cap, Pop, Terr, Infra, Money, Class, Hier, MSP, HS))   ###  Drop terms not in the best-by-AIC model
summary(fit <- lm(RD))

RD <- subset(RegrDat, select = -c(Cap, Pop, Terr, Infra, Money, Class, Hier, MSP, HS, Grain))   ###  Drop negative Grain term
summary(fit <- lm(RD))

RD <- subset(RegrDat, select = -c(Cap, Pop, Terr, Infra, Money, Class, Hier, MSP, HS, Grain, Info))   ###  Drop NS Info
summary(fit <- lm(RD))

#####################################################################################################
#### Check for the remaining predictors (those that substantially reduce the number of observations)
{### Irrigation: NS
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Gov, Agri, AgriLag, MilTech, IronCav, Irrigation))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, T, Lag2) ) 
  summary(fit <- lm(RegrDat))
}

{### Market: NS
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Gov, Agri, AgriLag, MilTech, IronCav, Market))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, T, Lag2) ) 
  summary(fit <- lm(RegrDat))
...
#####################################################################################################
{### Back to the best-fitting model
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Gov, Agri, AgriLag, MilTech, IronCav))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, Phylogeny, T, Lag2) ) 
  summary(fit <- lm(RegrDat))
}

### Check for Phylogeny: P = 0.001. Add the term to the model. AgriLag becomes NS, we drop it
RD <- cbind(RegrDat, NGARegrDat$Phylogeny)
colnames(RD)[ncol(RD)] <- "Phylogeny" 
summary(fit <- lm(RD))
RD <- subset(RD, select=-AgriLag)
summary(fit <- lm(RD))
...
### Check for NGA fixed effects: delAIC = 877.59 - 864.39 = 13.20. Add NGA.
...
{### Agri.sq: NS
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Gov, Agri, AgriLag, MilTech, IronCav, Agri.sq))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
...
{### MilTech.sq: NS
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Gov, Agri, AgriLag, MilTech, IronCav, MilTech.sq))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, T, Lag2, Phylogeny) ) 
  summary(fit <- lm(RegrDat))
...
{### Agri_MilTech: NS
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Gov, Agri, AgriLag, MilTech, IronCav, Agri_MilTech))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, T, Lag2, Phylogeny) ) 
  summary(fit <- lm(RegrDat))
...
#####################################################################################################
#### Revert to the best-fitting model, but without Phylogeny (which is knocked out in Phase II)
{###  R2 = 0.893
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Gov, Agri, MilTech, IronCav))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, Phylogeny, T, Lag2) )
  ...
  write.csv(out$coefficients, file="./outputs/1Anlz_Gov_output_3.csv")
  #write.table(out$coefficients, "clipboard", sep="\t")
}

{### R2 without autocorrelation term; R2 = 0.710
  RD <- subset(RegrDat, select = -c(Gov, Gov.sq, NGA))   
  # out for now
  # RD <- subset(RegrDat, select = -c(Gov, Gov.sq, NGA, Phylogeny)) 
  print(summary(fit <- lm(RD)))
  out <- summary(fit <- glm(RD1))
  write.csv(out$coefficients, file="./outputs/1Anlz_Gov_output_4.csv")
  #write.table(out$coefficients, "clipboard", sep="\t")
}
