...
ImpSCDatRepl <- read.table('./Data/ImpSCDatRepl5.csv', sep=",", stringsAsFactors = 	FALSE, header=TRUE)
...
for(iRep in 1:nRep){
  {#### Form data matrix
    ...
    AggrDat$Gov <- AggrSCWarAgriRelig$Gov                        ### Non-imputed for response variable
    ...
    ### Define the model 
    AggrDat <- subset(AggrDat, select = c(NGA, PolID, Time, Gov, Agri, MilTech, IronCav)) #******* define model here
...
write.csv(out, file="./outputs/2Boot_Gov_output_1.csv", row.names=FALSE)

{#########   Results
  out <- read.table("./outputs/2Boot_Gov_output_1.csv", sep=",", header=TRUE, stringsAsFactors = 	FALSE)
  n <- nrow(out)
  ...
  write.csv(BootSum, file="./outputs/2Boot_Gov_output_2.csv", row.names=FALSE)
