#####  Dynamic Regressions with Hier as response variable
{#### Get data
  ...
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Hier, Cap, Pop, Terr, Agri, AgriLag, Infra,
                                         Money, Info, Class, Grain, MilTech, IronCav, MSP, HS))
...
{########   
  ...
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, T, Phylogeny, Lag2) ) ## Drop non-sign autocorr terms
  ...
  {####  Exhaustive regressions with these terms
    ...
    write.csv(output, file="./outputs/1Anlz_Hier_output_1.csv",  row.names=FALSE)
    output <- output[output$delAIC < 2,]
    write.csv(output[1:min(100,nrow(output)),], file="./outputs/1Anlz_Hier_output_2.csv",  row.names=FALSE)
    #write.table(output[1:min(100,nrow(output)),], "clipboard", sep="\t", row.names=FALSE)
  }
}

#######  Regression coefficients
RD <- subset(RegrDat, select = -c(Cap, Pop, Terr, Infra, Money, Class, Grain, MSP))   ###  Drop terms not in the best-by-AIC model
summary(fit <- lm(RD))

RD <- subset(RegrDat, select = -c(Cap, Pop, Terr, Infra, Money, Class, Grain, MSP, HS, Info))   ###  Drop NS HS, Info
summary(fit <- lm(RD))
...
{### Irrigation: NS
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Hier, Agri, AgriLag, MilTech, IronCav, Irrigation))
...
{### Market: NS
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Hier, Agri, AgriLag, MilTech, IronCav, Market))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, Phylogeny, T, Lag2) ) 
  summary(fit <- glm(RegrDat))
...
#####################################################################################################
{### Retrun to the best-fitting model 
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Hier, Agri, AgriLag, MilTech, IronCav))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, Phylogeny, T, Lag2) ) 
  summary(fit <- glm(RegrDat))
...
### Check for NGA fixed effects: delAIC = 467.94 - 451.77 = 16.17. Add NGA. 
...
{#### Check for Lag2. P = 0.03, no effect on predictors; don't add to save df
TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Hier, Agri, AgriLag, MilTech, IronCav))
dpar <- 1000*1; source("./Data/fRegrDat.R")  
NGARegrDat <- RegrDat
RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, Phylogeny, T) ) 
...
{### Agri.sq: NS
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Hier, Agri, AgriLag, MilTech, IronCav, Agri.sq))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
...
{### MilTech.sq: NS
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Hier, Agri, AgriLag, MilTech, IronCav, MilTech.sq))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, T, Lag2, Phylogeny) ) 
  summary(fit <- lm(RegrDat))
...
{### Agri_MilTech: NS
  TableDat <- subset(AggrDat, select = c(NGA, PolID, Time, Hier, Agri, AgriLag, MilTech, IronCav, Agri_MilTech))
  dpar <- 1000*1; source("./Data/fRegrDat.R")  
  NGARegrDat <- RegrDat
  RegrDat <- subset(NGARegrDat, select = -c(NGA, PolID, Time, Space, T, Lag2, Phylogeny) ) 
  summary(fit <- lm(RegrDat))
...
{###  R2 = 0.905
  ...
  write.csv(out$coefficients, file="./outputs/1Anlz_Hier_output_3.csv")
  #write.table(out$coefficients, "clipboard", sep="\t")
}

{### R2 without autocorrelation term; R2 = 0.711
  RD <- subset(RegrDat, select = -c(Hier, Hier.sq, NGA)) 
  print(summary(fit <- lm(RD)))
  out <- summary(fit <- glm(RD1))
  write.csv(out$coefficients, file="./outputs/1Anlz_Hier_output_4.csv")
  #write.table(out$coefficients, "clipboard", sep="\t")
}
